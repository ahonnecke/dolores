...
``[options.entry_points]`` section in ``setup.cfg``::
    ...
    console_scripts =
         fibonacci = dolores.skeleton:run
...
from dolores import __version__, main
...
class SNSTopicListener:
    ...
    def remove_queue(self):
        try:
            ...
            _logger.info("Deleted queue with URL=%s.", self.sqs_queue.url)
        except ClientError as error:
            _logger.exception("Couldn't delete queue with URL=%s!", self.sqs_queue.url)
            raise error
